...
function renderUserRoleTable(role, tableId) {
    ...
    tbody.innerHTML = users.map((t, i) => {
        ...
        const roleText = t.rol === 'invitado' ? 'cliente' : (t.rol || 'tecnico');
        return `
            <tr>
                <td>${i + 1}</td>
                <td><strong>${t.nombre}</strong></td>
                <td>${t.email}</td>
                <td><span class="brand-badge">${roleText}</span></td>
                <td>
...
async function abrirModalTecnico(tecnico = null, role = 'tecnico') {
    ...
    const saveButton = document.getElementById('saveTecnico');
    const roleLabels = { tecnico: 'Técnico', admin: 'Administrador', invitado: 'Cliente' };

    const updateSaveLabel = currentRole => {
        if (saveButton) saveButton.textContent = `Guardar ${roleLabels[currentRole] || 'Usuario'}`;
    };

    if (tecnico) {
        title.innerHTML = `<i class="fas fa-user-edit"></i> Editar ${roleLabels[tecnico.rol] || 'Usuario'}`;
        ...
        title.innerHTML = role === 'admin'
            ...
            : role === 'invitado'
                ? '<i class="fas fa-user-tag"></i> Agregar Cliente'
    ...
    updateSaveLabel(tecnico?.rol || role);
...
document.getElementById('tecRol')?.addEventListener('change', function() {
    ...
    const roleLabels = { tecnico: 'Técnico', admin: 'Administrador', invitado: 'Cliente' };
    const saveButton = document.getElementById('saveTecnico');
    if (saveButton) saveButton.textContent = `Guardar ${roleLabels[this.value] || 'Usuario'}`;
});
