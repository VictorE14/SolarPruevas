...
(function () {
    ...
    async function getInversoresByUsuario(usuarioId) {
        ...
        const [{ data: links, error: linksError }, { data: legacy, error: legacyError }] = await Promise.all([
            client
            .from('inversores_tecnicos')
            .select('inversor_id, inversores(*)')
        ...
        (links || []).forEach(row => {
            if (row.inversores) merged.set(row.inversores.id, row.inversores);
        });
        (legacy || []).forEach(inversor => merged.set(inversor.id, inversor));
        return [...merged.values()].sort((a, b) => a.nombre.localeCompare(b.nombre));
    }

    async function getInversorIdsByUsuario(usuarioId) {
        const client = ensureClient();
        const { data, error } = await client
            .from('inversores_tecnicos')
            .select('inversor_id')
            .eq('usuario_id', usuarioId);
        if (error) throw error;
        return (data || []).map(row => row.inversor_id);
    }

    async function getInversorAssignmentCounts() {
        const client = ensureClient();
        const { data, error } = await client
            .from('inversores_tecnicos')
            .select('usuario_id');
        ...
        return (data || []).reduce((counts, row) => {
            counts[row.usuario_id] = (counts[row.usuario_id] || 0) + 1;
            return counts;
        }, {});
    }

    async function getInversorAssignments() {
        const client = ensureClient();
        const { data, error } = await client
            .from('inversores_tecnicos')
            .select('inversor_id, usuario_id');
        if (error) throw error;

        return (data || []).reduce((assignments, row) => {
            if (!assignments[row.inversor_id]) assignments[row.inversor_id] = [];
            assignments[row.inversor_id].push(row.usuario_id);
            return assignments;
        }, {});
    }

    async function setInversoresForUsuario(usuarioId, inversorIds) {
        const client = ensureClient();
        const uniqueIds = [...new Set((inversorIds || []).filter(Boolean))];
        ...
        const { error: insertError } = await client
            .from('inversores_tecnicos')
            .insert(uniqueIds.map(inversorId => ({ inversor_id: inversorId, usuario_id: usuarioId })));
        if (insertError) throw insertError;
        return true;
    }

    async function assignInversorToUsuario(inversorId, usuarioId) {
        const client = ensureClient();
        if (!inversorId || !usuarioId) return true;
        const { error } = await client
            .from('inversores_tecnicos')
            .upsert({ inversor_id: inversorId, usuario_id: usuarioId }, { onConflict: 'inversor_id,usuario_id', ignoreDuplicates: true });
        if (error) throw error;
        return true;
    }

    async function unassignInversorFromUsuario(inversorId, usuarioId) {
        const client = ensureClient();
        if (!inversorId || !usuarioId) return true;
        const { error } = await client
            .from('inversores_tecnicos')
    ...
    async function sincronizarGrowatt(inverterId) {
        ...
        if (error) {
            ...
            try {
                ...
                console.warn('No se pudo leer el detalle del error Growatt:', parseError);
            }
    ...
    window.db = {
        ...
        getInversorIdsByUsuario,
        getInversorAssignmentCounts,
        getInversorAssignments,
        setInversoresForUsuario,
        assignInversorToUsuario,
        unassignInversorFromUsuario,
        getAllInversores,
        createInversor,
        updateInversor,
        deleteInversor,
